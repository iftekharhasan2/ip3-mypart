...
import { MerlaService } from './services/MerlaService';
...
const ALL_SERVICE_TABS = [
  { id: 'economic', label: 'Economic Assessment', icon: TrendingUp, number: '01' },
  { id: 'climate', label: 'Climate Action & ESG', icon: Globe2, number: '02' },
  { id: 'design', label: 'Program & Survey Design', icon: ClipboardCheck, number: '03' },
  { id: 'merla', label: 'Monitoring & MERLA', icon: BarChart3, number: '04' },
  { id: 'macro-policy', label: 'Macro & Sector Advisory', icon: Building2, number: '05' },
  { id: 'digital-systems', label: 'Digital Systems', icon: Cpu, number: '06' },
  { id: 'capacity-building', label: 'Institutional Capacity', icon: GraduationCap, number: '07' },
];
...
export const ServicesPage: React.FC<ServicesPageProps> = ({
  ...
  const resolveSubPage = (id?: string) => {
    if (!id) return 'economic';
    const clean = id.replace(/^#/, '').toLowerCase();
    if (clean.includes('eco') || clean.includes('poilicy_eco')) return 'economic';
    if (clean.includes('climate') || clean.includes('policy_climate')) return 'climate';
    if (clean.includes('design') || clean.includes('policy_design')) return 'design';
    if (clean.includes('merla') || clean.includes('mon') || clean.includes('policy_mon')) return 'merla';
    const found = SERVICES_SUB_PAGES.find(s => s.id === clean || s.slug === clean);
    if (found) return found.id;
    return 'economic';
  };

  const [activeSubPageId, setActiveSubPageId] = useState<string>(() => resolveSubPage(initialSubPageId));

  useEffect(() => {
    if (initialSubPageId) {
      setActiveSubPageId(resolveSubPage(initialSubPageId));
  ...
  return (
    <div className="min-h-screen bg-[#050a12] text-slate-100 font-sans selection:bg-[#ff7e67]/30 selection:text-[#ff9d8c] pb-24">
      ...
      {/* Render Full Converted Service Components */}
      {activeSubPageId === 'economic' ? (
        <EconomicService
          onOpenCollaborate={onOpenCollaborate}
          onOpenTalk={onOpenTalk}
          onNavigateContact={onNavigateContact}
        />
      ) : activeSubPageId === 'climate' ? (
        <ClimateService
          onOpenCollaborate={onOpenCollaborate}
          onOpenTalk={onOpenTalk}
          onNavigateContact={onNavigateContact}
        />
      ) : activeSubPageId === 'design' ? (
        <DesignService
          onOpenCollaborate={onOpenCollaborate}
          onOpenTalk={onOpenTalk}
          onNavigateContact={onNavigateContact}
        />
      ) : activeSubPageId === 'merla' ? (
        <MerlaService
          onOpenCollaborate={onOpenCollaborate}
          onOpenTalk={onOpenTalk}
          onNavigateContact={onNavigateContact}
